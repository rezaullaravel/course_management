      ...
      <div class="row">
         <div class="col-sm-12">
            <div class="card">
               ...
               <div class="card-body">
                  <form action="{{route('admin.course-post.update',$course->id)}}" method="POST" enctype="multipart/form-data">
                     @csrf
                     <div class="form-group">
                        ...
                        <select name="course_category_id" class="form-control">
                           ...
                           <option value="{{ $row->id }}" {{ $course->course_category_id == $row->id ? 'selected':'' }}>{{ $row->name }}</option>
                           @endforeach
                        </select>
                     ...
                     <div class="form-group">
                        ...
                        <input type="text" name="title" value="{{ $course->title }}"  class="form-control" placeholder="Enter course title" rows="5">
                        @error('title')
                        <span class="text-danger">{{ $message }}</span>
                        @enderror
                     </div>
                     <div class="form-group">
                        ...
                        <textarea  class="form-control" id="editor" name="content"  rows="5" >{{ $course->content }}</textarea>
                        @error('content')
                        <span class="text-danger">{{ $message }}</span>
                        @enderror
                     </div>
                     <div class="form-group">
                        ...
                        <span class="text-danger">{{ $message }}</span>
                        @enderror
                        <img src="{{ $course->image ? asset($course->image) : '' }}" id="postImage" style="margin-top: 5px;">
                     </div>
