    ...
    <div class="w-100 h-100 d-flex justify-content-center align-items-center">
        ...
        <div class="modal-card border-0">
            ...
            @if (session()->get('discount_amount_bn'))
                @php
                    $total = $package->price_bn - session()->get('discount_amount_bn');
                @endphp
            @else
                @php
                    $total = $package->price_bn;
                @endphp
            @endif
            <div class="gap-40 flex-lg-row">
                <!-- Personal details -->
                <form action="{{ route('package-order.store') }}" method="POST">
                    @csrf

                    <input type="hidden" name="order_total_bn" value="{{ $total }}">
                    <input type="hidden" name="package_id" value="{{ $package->id }}">
                    <input type="hidden" name="price_bn" value="{{ $package->price_bn }}">

                    <div class="billing-details gap-32 padding-40 card">
                        <div class="d-flex justify-content-between">
                            <h5>প্যাকেজ  অর্ডার সংক্রান্ত তথ্য</h5>
                    ...
                    <div class="gap-32">
                        ...
                        <div class="gap-32 flex-row p-1 justify-content-between align-items-center">
                            <div class="gap-16 flex-row align-items-center">
                                ...
                                <p class="p-16">{{ $package->name_bn }}</p>
                            </div>

                            <p class="p-18 fw-medium">৳ {{ $package->price_bn }}</p>
                        </div>
                        ...
                        <div class="gap-16">
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <p class="p-18 color-text">
                                    @if (session()->get('discount_amount_bn'))
                                        ৳ {{ $package->price_bn - session()->get('discount_amount_bn') }}
                                    @else
                                        ৳ {{ $package->price_bn }}
                                    @endif
                                </p>
